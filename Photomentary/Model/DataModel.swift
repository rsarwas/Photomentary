...
final class DataModel: ObservableObject {
  ...
  func start() {
    displayTimer = Timer.TimerPublisher(
      interval: displayInterval, tolerance: 0.5, runLoop: .main, mode: .common
    )
    .autoconnect()
  ...
  func next() {
    ...
    if (currentPointer + 1) % photos.count == insertPointer {
      print(
        "Waiting for new photos to load. currentPointer: \(currentPointer), insertPointer: \(insertPointer), photos.count: \(photos.count), cacheSize: \(cacheSize)"
      )
  ...
  func previous() {
    if currentPointer == insertPointer || (photos.count < cacheSize && currentPointer == 0) {
      print(
        "Can't back up any further. currentPointer: \(currentPointer), insertPointer: \(insertPointer), photos.count: \(photos.count), cacheSize: \(cacheSize)"
      )
